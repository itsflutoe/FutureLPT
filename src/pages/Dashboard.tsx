...
export default function Dashboard() {
  ...
  const major = Object.entries(subjects).filter(([, v]) => v.category === 'SPECIALIZATION');
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {(major.length > 0 || true) && (
          <Card className="lg:col-span-2">
            ...
            <CardContent className="space-y-3">
              {major.length === 0 ? (
                <p className="text-sm text-[var(--muted-foreground)]">No data yet. Practice specialization topics to see performance.</p>
              ) : (
                major.map(([name, v]) => (
                  <div key={name}>
                    <div className="flex justify-between text-sm mb-1">
